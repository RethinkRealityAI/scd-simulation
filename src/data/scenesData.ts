...
export const defaultSceneLayoutConfig: SceneLayoutConfig = {
  components: [
    { id: 'vitals-monitor', type: 'vitals-monitor', enabled: true, x: 0, y: 0, w: 3, h: 10, minW: 2, minH: 5 },
    ...
    { id: 'interactive-panel', type: 'interactive-panel', enabled: true, x: 9, y: 2, w: 3, h: 8, minW: 3, minH: 4 },
...
export function normalizeSceneLayoutConfig(layoutConfig?: unknown): SceneLayoutConfig {
  ...
  // Legacy migration: merge quiz-panel + action-prompt → interactive-panel
  if (!persistedByType.has('interactive-panel')) {
    const quizEntry = persistedByType.get('quiz-panel');
    const actionEntry = persistedByType.get('action-prompt');
    if (quizEntry || actionEntry) {
      const base = quizEntry || actionEntry;
      persistedByType.set('interactive-panel', {
        ...base,
        id: 'interactive-panel',
        type: 'interactive-panel',
        enabled: (quizEntry?.enabled !== false) || (actionEntry?.enabled !== false),
      });
    }
  }
  persistedByType.delete('quiz-panel');
  persistedByType.delete('action-prompt');
...
export const scenes: SceneData[] = [
  ...
  },
  {
    ...
    layoutConfig: {
      components: [
        { id: 'vitals-monitor', type: 'vitals-monitor', enabled: false, x: 0, y: 0, w: 3, h: 10, minW: 2, minH: 5 },
        { id: 'scene-header', type: 'scene-header', enabled: true, x: 0, y: 0, w: 12, h: 2, minW: 4, minH: 1, maxH: 3 },
        { id: 'video-player', type: 'video-player', enabled: false, x: 3, y: 2, w: 6, h: 6, minW: 3, minH: 3 },
        { id: 'clinical-findings', type: 'clinical-findings', enabled: false, x: 3, y: 8, w: 6, h: 2, minW: 2, minH: 1 },
        { id: 'interactive-panel', type: 'interactive-panel', enabled: true, x: 0, y: 2, w: 12, h: 8, minW: 6, minH: 4 },
        { id: 'audio-player', type: 'audio-player', enabled: false, x: 3, y: 8, w: 6, h: 2, minW: 3, minH: 1 },
      ],
    },
  },
