...
/** Demo data shown when the component is rendered outside a SimulationProvider (e.g. admin preview) */
const PREVIEW_SCORE = 73;
const PREVIEW_CATEGORY_SCORES: CategoryScore[] = [
...
const CompletionResults: React.FC = () => {
  const simulationCtx = useContext(SimulationContext);
  const [animationStep, setAnimationStep] = useState(0);

  // Safe helpers — fall back to demo values when there's no provider
  const score: number = simulationCtx ? simulationCtx.calculateScore() : PREVIEW_SCORE;
  const categoryScores: CategoryScore[] = simulationCtx
    ? simulationCtx.calculateCategoryScores()
    : PREVIEW_CATEGORY_SCORES;
  const completionTime: number =
    simulationCtx && simulationCtx.state.userData.responses.length > 0
      ? Date.now() - simulationCtx.state.userData.startTime
      : 0;
  ...
  const getPerformanceLevel = (s: number) => {
    if (s >= 90) return { level: 'Excellent', description: 'Outstanding understanding of sickle cell crisis management', icon: Award, color: 'text-green-400' };
    if (s >= 80) return { level: 'Very Good', description: 'Strong grasp of key concepts and protocols', icon: Trophy, color: 'text-blue-400' };
    if (s >= 70) return { level: 'Good', description: 'Solid foundation with room for improvement', icon: Target, color: 'text-cyan-400' };
    if (s >= 60) return { level: 'Satisfactory', description: 'Basic understanding, additional training recommended', icon: TrendingUp, color: 'text-yellow-400' };
    return { level: 'Needs Improvement', description: 'Further education and practice strongly recommended', icon: AlertCircle, color: 'text-red-400' };
  };
  ...
  const categoryConfig: Record<ScoringCategory, { label: string; description: string; icon: React.FC<{ className?: string }>; color: string; bgColor: string; borderColor: string }> = {
    timelyPainManagement: { label: 'Timely Pain Management', description: 'Rapid assessment and appropriate pain interventions', icon: Heart, color: 'text-red-400', bgColor: 'bg-red-500/10', borderColor: 'border-red-400/30' },
    clinicalJudgment: { label: 'Clinical Judgment', description: 'Evidence-based decision making and risk assessment', icon: Brain, color: 'text-blue-400', bgColor: 'bg-blue-500/10', borderColor: 'border-blue-400/30' },
    communication: { label: 'Communication', description: 'Effective interprofessional and patient communication', icon: MessageSquare, color: 'text-green-400', bgColor: 'bg-green-500/10', borderColor: 'border-green-400/30' },
    culturalSafety: { label: 'Cultural Safety', description: 'Culturally responsive and respectful care delivery', icon: Shield, color: 'text-purple-400', bgColor: 'bg-purple-500/10', borderColor: 'border-purple-400/30' },
    biasMitigation: { label: 'Bias Mitigation', description: 'Recognition and addressing of healthcare biases', icon: Shield, color: 'text-purple-400', bgColor: 'bg-purple-500/10', borderColor: 'border-purple-400/30' },
  };
              ...
              <div className="p-2 rounded-lg bg-slate-800/30">
                <div className="flex items-center justify-between">
                  ...
                  <span className="text-cyan-400 font-bold text-sm">
                    {completionTime > 0 ? formatTime(completionTime) : '—'}
                  </span>
                </div>
              </div>
