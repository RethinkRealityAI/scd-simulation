...
const SceneManagementDashboard: React.FC<SceneManagementDashboardProps> = ({ onClose, instanceId }) => {
  ...
  // Restore editor-open state from session so tab-switching doesn't reset
  ...
  useEffect(() => {
    ...
    try {
      ...
      const match = allScenes.find(s => s.id === stored.sceneId);
      if (match) {
        setSelectedScene(match);
        setInitialBuilderMode(stored.mode || 'canvas');
        setShowEditor(true);
      }
    } catch { /* ignore corrupt data */ }
  }, [allScenes]);

  // Persist current editor context whenever it changes
  useEffect(() => {
    try {
