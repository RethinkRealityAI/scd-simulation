...
const INPUT_CLS = 'w-full px-3 py-2 border border-gray-200 rounded-lg text-sm bg-gray-50 focus:bg-white focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-colors placeholder-gray-400 text-gray-900';
...
function VitalsConfig({
  ...
  const VITALS = [
    ...
    { key: 'painLevel' as const, visKey: 'painLevel' as const, label: 'Pain Level', unit: '/10', step: '1', optional: true },
  ] as const;
    ...
    <div className="space-y-4">
      ...
      <div>
        ...
        <button
          ...
          className={`flex items-center gap-2 px-3 py-2 rounded-lg border-2 transition-all w-full text-left text-sm ${
            sceneData.vitals.isAlarmOn
                  ...
                  <div className="flex gap-1 mt-1.5 ml-8 flex-wrap">
                    {VITAL_COLORS.map(color => (
                      <button
                        ...
                        className={`w-4 h-4 rounded-full ${COLOR_SWATCHES[color]} transition-all ${
  ...
  if (!selectedType) {
    return (
      <div className="flex flex-col items-center justify-center h-full text-center p-6">
        ...
        <p className="text-sm font-semibold text-gray-700">No component selected</p>
