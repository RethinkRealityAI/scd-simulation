...
// Component types that especially benefit from the full-width editor
...
const RightPanel: React.FC<RightPanelProps> = ({
  ...
  return (
    <div className="flex-shrink-0 w-80 flex flex-col bg-white border-l border-gray-200 overflow-hidden">
      {/* ── Tab bar ──────────────────────────────────────────────────────── */}
      <div className="flex-shrink-0 flex border-b border-gray-200 bg-gray-50">
        {TABS.map(({ id, label, icon: Icon }) => (
          <button
            ...
            className={`flex-1 flex flex-col items-center gap-0.5 px-2 py-2.5 text-xs font-medium transition-colors border-b-2 ${
              activeTab === id
      ...
      <div className="flex-1 overflow-hidden flex flex-col">
        ...
        {activeTab === 'selected' && (
          <div className="flex flex-col h-full overflow-hidden">
            {canOpenDeepEditor && (
              <div className="flex-shrink-0 px-3 py-2 border-b border-blue-100 bg-blue-50">
                <button
                  onClick={onOpenDeepEditor}
                  className="w-full flex items-center justify-center gap-1.5 px-3 py-2 bg-blue-600 hover:bg-blue-700 text-white rounded-lg text-xs font-semibold transition-colors shadow-sm"
                >
            ...
            <div className="flex-1 overflow-y-auto p-3 space-y-4">
              ...
              <div className="p-3 rounded-xl bg-gray-50 border border-gray-200">
                ...
                <div className="text-xs text-gray-500 space-y-0.5">
                  ...
                  <div>Prompts: <span className="font-medium text-gray-700">{sceneData.actionPrompt?.type ?? 'none'}</span></div>
                </div>
              </div>
            </div>
          </div>
